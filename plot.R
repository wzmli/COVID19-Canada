...
ddtoday <- (ddtoday 
	...
	%>% mutate(Province = ifelse(Province == "YT", "YT,NT,NU", Province)
		, Province = ifelse(Province %in% c("NT","NU","PEI","NB"), "", Province)
		, Province = ifelse(Province == "NL","NL,PEI,NB", Province)
		)
	%>% ungroup()
	%>% arrange(newTests)
)
...
ggtoday <- (ggplot(ddtoday, aes(x=newTests))
	...
	+ xlim(c(1,7000))
	+ ylim(c(0.01,800))
	+ scale_x_log10(breaks=c(1,ddtoday$newTests[c(1,2,3,4,6,8,9,10,11,12,13)]))
