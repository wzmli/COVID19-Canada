...
#	%>% mutate(Province = ifelse(Province == "NS", "NS,SK", Province)
#		, Province = ifelse(Province %in% c("SK","MB"), "", Province)
#		, Province = ifelse(Province == "NB","NB,MB", Province)
#		)
	%>% ungroup()
	%>% arrange(newTests)
)
...
ggtoday <- (ggplot(ddtoday, aes(x=newTests))
	...
	+ xlim(c(1,7000))
	+ ylim(c(0.01,800))
	+ scale_x_log10(breaks=c(1,ddtoday$newTests[c(1,3,4,6,7,8,9,10,11,12,13)]))
