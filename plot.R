...
ddtoday <- (ddtoday 
	...
	%>% mutate(Province = ifelse(Province == "NL", "NL,NB,BS,PEI,MB", Province)
		, Province = ifelse(Province %in% c("NS","NB","PEI","MB"), "", Province)
...
ggtoday <- (ggplot(ddtoday, aes(x=newTests))
	...
	+ xlim(c(1,7000))
	+ ylim(c(0.01,800))
	+ scale_x_log10(breaks=c(1,ddtoday$newTests[c(1,2,3,4,5,6,8,9,10,11,12,13)]))
